# Cyber talents_Introduction to Cybersecurity

 solution for basic challenges of Introduction to Cybersecurity section in cyber talent platform.

 you can see the challenges at [Introduction to Cybersecurity](https://cybertalents.com/learn/introduction-to-cybersecurity)

this section is consists of 28 part cover various branches of information security.

in this Repo. you will find each challenge with the correct flag and for the writeup will be added to [this Repo](https://github.com/Zer0LorD/Cyber-talents).

---

---

## [1- introduction to ctf :-](https://cybertalents.com/learn/introduction-to-cybersecurity/1-introduction-to-ctf/)

- [Pay Me](https://cybertalents.com/challenges/general-information/pay-me)
  
```text
I will lock your machine screen or files till you pay me. Who am i ?
```

>ransomware

---

## [2- introduction to web security:-](https://cybertalents.com/learn/introduction-to-cybersecurity/2-introduction-to-web-security/)

- [Hackers Gathering](https://cybertalents.com/challenges/general-information/hackers-gathering)
  
```text
I am a cyber security conference that run in August every year in Las Vegas. I am the largest gathering for Hackers in the whole world. No Credit cards, no online booking , Only Cash allowed . Who am I ?
```

>defcon

---

## [3- html :-](https://cybertalents.com/learn/introduction-to-cybersecurity/3-html/)

- [htmlentities](https://cybertalents.com/challenges/general-information/htmlentities)
  
```text
True or False , htmlentities ( convert special characters to its html entity ) can't be exploited to run XSS payload ?
```

>False

---

## [4- javascript :-](https://cybertalents.com/learn/introduction-to-cybersecurity/4-javascript/)

- [this is sparta](https://cybertalents.com/challenges/web/this-is-sparta)
  
```text
Morning has broken today they're fighting in the shade when arrows blocked the sun they fell tonight they dine in hell
```

***[challenge link](http://35.193.45.56/sparta/)***

>{J4V4_Scr1Pt_1S_Aw3s0me}

---

## [5- cookies :-](https://cybertalents.com/learn/introduction-to-cybersecurity/5-cookies/)

- [Admin has the power](https://cybertalents.com/challenges/web/admin-has-the-power)
  
```text
Administrators only has the power to see the flag , can you be one ?
```

***[challenge link](http://35.193.45.56/adminpower/)***

>hiadminyouhavethepower

---

## [6- hashing :-](https://cybertalents.com/learn/introduction-to-cybersecurity/6-hashing/)

- [Hash3rror](https://cybertalents.com/challenges/cryptography/hash3rror)
  
```text
we got this corrupted hash password from a Pcap file with a note (password = sha-1(hash-result)).

HASH:77be5d24ed2e3e590045e1d6o7e84i50d2799c19f48ede46804a8734e287df120f 
```

>83874343435092cb681c0d558a84bfeb389c32ed

---

## [7- encoding :-](https://cybertalents.com/learn/introduction-to-cybersecurity/7-encoding/)

- [who am i?](https://cybertalents.com/challenges/web/who-am-i)
  
```text
Do not Start a fight you can not stop it
```

***[challenge link](http://34.76.107.218/whoami/)***

>FLag{B@D_4uTh1Nt1C4Ti0n}

---

## [8- obfuscation :-](https://cybertalents.com/learn/introduction-to-cybersecurity/8-obfuscation/)

- [Modify Code](https://cybertalents.com/challenges/general-information/modify-code)
  
```text
Change code from one form to another to prevent attacker from understanding it
```

>Obfuscation

---

- [Iam Legend](https://cybertalents.com/challenges/web/iam-legend)
  
```text
If I am a legend, then why am I so lonely?

Flag Format : FLAG{}
```

***[challenge link](http://35.193.45.56/iamlegend/)***

>flag{J4V4_Scr1Pt_1S_S0_D4MN_FUN}

---

## [9- XSS :-](https://cybertalents.com/learn/introduction-to-cybersecurity/9-xss/)

- [Searching for the cookie](https://cybertalents.com/challenges/web/searching-for-the-cookie)
  
```text
simple search website we need to know which cookie to eat ;)
```

***[challenge link](http://34.77.37.110/searching-cookie/)***

>coolcookie112

---

## [10- directory traversal :-](https://cybertalents.com/learn/introduction-to-cybersecurity/10-directory-traversal/)

- [bean](https://cybertalents.com/challenges/web/bean)
  
```text
Come back home Mr Bean .
```

***[challenge link](http://52.28.216.196/bean/)***

>FLAG{Nginx_nOt_aLWays_sEcUre_bY_The_waY}

---

## [11- sensitive data exposure :-](https://cybertalents.com/learn/introduction-to-cybersecurity/sensitive-data-exposure/)

- [Maximum Courage](https://cybertalents.com/challenges/web/maximum-courage)
  
```text
Max prefers to learn by practicing and not just reading all day, so he set up a webserver and hopes it stays secret, can you prove it has a weakness?
```

***[challenge link](http://3.126.138.80/maximum/)***

>be607453caada6a05d00c0ea0057f733

---

## [12- burp suite :-](https://cybertalents.com/learn/introduction-to-cybersecurity/burp-suite/)

- [The Restricted Sessions](https://cybertalents.com/challenges/web/the-restricted-sessions)
  
```text
Flag is restricted to logged users only , can you be one of them.
```

***[challenge link](http://34.77.37.110/restricted-sessions/)***

>sessionareawesomebutifitsecure

---

## [13- command injection :-](https://cybertalents.com/learn/introduction-to-cybersecurity/13-command-injection/)

- [Newsletter](https://cybertalents.com/challenges/web/newsletter)
  
```text
the administrator put the backup file in the same root folder as the application, help us download this backup by retrieving the backup file name
```

***[challenge link](http://18.192.3.151/newsletter/)***

>hgdr64.backup.tar.gz

---

## [14- code injection :-](https://cybertalents.com/learn/introduction-to-cybersecurity/code-injection/)

- [Hashable](https://cybertalents.com/challenges/web/hashable)
  
```text
A famous enterprise blog was hacked, can you figure out how it was hacked?
```

***[challenge link](http://ec2-35-158-236-11.eu-central-1.compute.amazonaws.com/hashable/)***

>18ab51f960bd149bcb2497b9998c752c

---

## [15- sql injection :-](https://cybertalents.com/learn/introduction-to-cybersecurity/sql-injection/)

- [Easy access](https://cybertalents.com/challenges/web/easy-access)
  
```text
Only superpower makes you see unlimited view.
```

***[challenge link](http://ec2-35-158-236-11.eu-central-1.compute.amazonaws.com/easy-access/)***

>flag{!njection_3v3ry_wh3r3}

---

## [16- introduction to digital forensics :-](https://cybertalents.com/learn/introduction-to-cybersecurity/16-introduction-to-digital-forensics/)

- [Crime scene](https://cybertalents.com/challenges/general-information/crime-scene)
  
```text
Process of analyzing and investing computer devices, on suspecting that such devices may have been used in a cybercrime
```

>forensics

---

## [17- meta data :-](https://cybertalents.com/learn/introduction-to-cybersecurity/17-meta-data/)

- [Keep it Simple](https://cybertalents.com/challenges/forensics/keep-it-simple)
  
```text
The answer is simple
```

>{S1mpl3sty_i$_th_@nswer}

---

## [18- steganography :-](https://cybertalents.com/learn/introduction-to-cybersecurity/18-steganography)

- [Greeks](https://cybertalents.com/challenges/general-information/greeks)
  
```text
The art of hiding messages or information inside other image / text or data
```

>Steganography

---

## [19- audio forensics :-](https://cybertalents.com/learn/introduction-to-cybersecurity/19-audio-forensics-1)

- [I love music](https://cybertalents.com/challenges/forensics/i-love-music)
  
```text
listen and focus , you will listen another thing
```

***[challenge link](https://s3-eu-west-1.amazonaws.com/hubchallenges/Forensics/love-music.wav)***

>you_are_victorious!

---

## [20- wireshark :-](https://cybertalents.com/learn/introduction-to-cybersecurity/20-wireshark)

- [Capture](https://cybertalents.com/challenges/general-information/capture)
  
```text
Network analysis tool used to captured packets and present it in readable format
```

>Wireshark

---

- [bflag](https://cybertalents.com/challenges/forensics/bflag)
  
```text
All of us started from the bottom. Now it's your turn.
```

***[challenge link](https://hubchallenges.s3-eu-west-1.amazonaws.com/Forensics/bflag.pcap)***

>analyze_packet_for_fun

---

## [21- netcat :-](https://cybertalents.com/learn/introduction-to-cybersecurity/21-netcat)

- [Cypher Anxiety](https://cybertalents.com/challenges/forensics/cypher-anxiety)
  
```text
An image was leaked from a babies store. the manager is so annoyed because he needs to identify the image to fire charges against the responsible employee. the key is the md5 of the image
```

***[challenge link](https://s3-eu-west-1.amazonaws.com/talentchallenges/Forensics/find+the+image.zip)***

>3beef06be834f3151309037dde4714ec

---

## [22- memory-forensics :-](https://cybertalents.com/learn/introduction-to-cybersecurity/22-memory-forensics)

- [Monaliza](https://cybertalents.com/challenges/forensics/monaliza)
  
```text
We cannot identify this suspicious user behavior, but we know he is a big fan of Monaliza
```

***[challenge link](https://hubchallenges.s3-eu-west-1.amazonaws.com/Forensics/monaliza.7z)***

>FLAG{P@!nting_i5_4wes0m3}

---

## [23- introduction to reverse engineering :-](https://cybertalents.com/learn/introduction-to-cybersecurity/23-introduction-to-reverse-engineering)

- [Find the Pass](https://cybertalents.com/challenges/malware/Find-the-pass)
  
```text
Answer the following Questions :

Q1- What is the address of the main function ?

Q2- What is the address of the function that Validate the input ?

Q3- What is the address of the hardcoded password ?

Flag format : flag{Ans1|Ans2|Ans3}
```

***[challenge link](https://talentchallenges.s3-eu-west-1.amazonaws.com/Reverse/CybertalentDemo0.exe)***

>flag{0x401180|0x401100|0x4020F8}

---

## [24- intel x86 architecture cover :-](https://cybertalents.com/learn/introduction-to-cybersecurity/24-intel-x86-architecture-cover)

- [bronze ASM](https://cybertalents.com/challenges/malware/asm-parameter)
  
```text
the flag is the parameter of the function int he following format ("FLAG{0_%X_0}" % parmter)
```

***[challenge link](https://hubchallenges.s3-eu-west-1.amazonaws.com/Reverse/ASM_Par.asm)***

>FLAG{0_2CE_0}

---

## [25- introduction to cryptography :-](https://cybertalents.com/learn/introduction-to-cybersecurity/25-introduction-to-cryptography)

- [security model](https://cybertalents.com/challenges/general-information/security-model)
  
```text
is a model designed to guide policies for information security within an organization
```

>CIA

---

## [26- morse code :-](https://cybertalents.com/learn/introduction-to-cybersecurity/26-morse-code)

- [LOUDER](https://cybertalents.com/challenges/cryptography/louder)
  
```text
Can you decode the wired sounds? 

Flag format: flag{X XX XXXXXXXX XXXXXX XXXX XXXXXX}
```

***[challenge link](https://hubchallenges.s3-eu-west-1.amazonaws.com/Crypto/morse.wav)***

>flag{I AM SPEAKING LOUDER THAN BEFORE}

---

## [27- caesar cipher :-](https://cybertalents.com/learn/introduction-to-cybersecurity/27-caesar-cipher)

- [Hide Data](https://cybertalents.com/challenges/cryptography/hide-data)
  
```text
I used to hide my data with a classic cypher, can you get the flag hidden inside?
gur synt vf 2w68lsudym Vg vf cerggl rnfl gb frr gur synt ohg pna lbh frr vg v gbbx arneyl 1 zvahgr gb rapbqr guvf jvgu EBG13 tbbq yhpx va fbyivat gung

```

>2j68yfhqlz

---

## [28- hash cracking :-](https://cybertalents.com/learn/introduction-to-cybersecurity/28-hash-cracking)

- [Guess The Password](https://cybertalents.com/challenges/cryptography/guess-the-password)
  
```text
A hacker leaked the below hash online.Can you crack it to know the password of the CEO? the flag is the password Hash: 06f8aa28b9237866e3e289f18ade19e1736d809d
```

>jrahyn+

---

that was all section solution see U in another challenge
